<script setup lang="ts">
  ...
  function zoneClass(position: number): string {
    ...
    if (competition.value === 'serie_a') {
      if (position <= 4) return 'border-l-4 border-l-neon bg-neon/5'
      if (position <= 6) return 'border-l-4 border-l-gold bg-gold/5'
      if (position >= 17) return 'border-l-4 border-l-live bg-live/5'
    }
...
<template>
  <div v-if="pending" class="space-y-3 py-4">
    <div v-for="i in 5" :key="i" class="h-10 animate-pulse rounded bg-card-raised" />
  </div>
  <BaseEmptyState v-else-if="!standings.length" :message="t('common.noResults')" />
  <div v-else class="overflow-x-auto">
    <table class="w-full text-left text-sm">
      <thead>
        <tr class="bg-card-raised font-display text-[10px] font-bold uppercase tracking-widest text-secondary">
      ...
      <tbody>
        <tr
          ...
          class="border-b border-theme transition-colors hover:bg-[var(--bg-surface-raised)] animate-fade-in"
          :class="[
            zoneClass(entry.position),
            favoriteSet.has(entry.teamExternalId) && 'bg-gold/5',
            `stagger-${(index % 8) + 1}`,
          ]"
        >
          <td class="px-2 py-2 text-center font-display font-bold text-primary">{{ entry.position }}</td>
          <td class="px-2 py-2">
            <div class="flex items-center gap-2">
              ...
              <span
                class="truncate"
                :class="favoriteSet.has(entry.teamExternalId) ? 'font-bold text-primary' : 'font-medium text-primary'"
              >
                {{
                  compact
          ...
          <td class="px-2 py-2 text-center text-secondary">{{ entry.played }}</td>
          <template v-if="!compact">
            <td class="hidden px-2 py-2 text-center text-secondary md:table-cell">{{ entry.won }}</td>
            <td class="hidden px-2 py-2 text-center text-secondary md:table-cell">{{ entry.drawn }}</td>
            <td class="hidden px-2 py-2 text-center text-secondary md:table-cell">{{ entry.lost }}</td>
            <td class="hidden px-2 py-2 text-center text-secondary md:table-cell">{{ entry.goalsFor }}</td>
            <td class="hidden px-2 py-2 text-center text-secondary md:table-cell">{{ entry.goalsAgainst }}</td>
          </template>
          <td class="px-2 py-2 text-center text-secondary">
            {{ entry.goalDifference > 0 ? `+${entry.goalDifference}` : entry.goalDifference }}
          </td>
          <td class="px-2 py-2 text-center font-display text-xl font-bold text-neon">{{ entry.points }}</td>
