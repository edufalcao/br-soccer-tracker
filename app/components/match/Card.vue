...
<template>
  <BaseCard
    class="!p-3"
    :class="[
      isFavoriteMatch && 'ring-2 ring-accent/30 !bg-accent/5',
      isLive && 'animate-live-glow ring-1 ring-red-200 !bg-red-50/50',
      isFinished && '!bg-pitch-50/30',
    ]"
  >
    <div class="flex items-center gap-3">
      ...
      <div class="flex flex-1 items-center justify-end gap-2 text-right">
        ...
        <span class="truncate text-sm font-semibold">{{ getTeamName(match.homeTeamId) }}</span>
      ...
      <div class="flex flex-col items-center gap-1">
        <span
          ...
          :class="isLive ? 'text-2xl text-red-600' : 'text-lg'"
        >
          {{ scoreDisplay }}
        </span>
        <MatchStatusBadge
      ...
      <div class="flex flex-1 items-center gap-2">
        ...
        <span class="truncate text-sm font-semibold">{{ getTeamName(match.awayTeamId) }}</span>
        <FavoriteStarIcon v-if="isFavorite(match.awayTeamId)" size="xs" />
      </div>
    </div>

    <!-- Venue -->
    <p v-if="match.venue" class="mt-1 text-center text-xs text-pitch-400">{{ match.venue }}</p>
  </BaseCard>
</template>
